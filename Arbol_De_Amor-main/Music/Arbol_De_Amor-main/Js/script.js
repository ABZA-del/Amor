...
let divarbol = document.getElementById('tree-container');
divarbol.addEventListener('click', function reproducir_musica(){
...
function showDedicationText() { //seguidores
  ...
  if (!text) {
    text = `Hola mi amor:\n\n No sabes lo feliz que me hace saber que puedo despertar cada dia a tu lado\nQue lo primero que vea al abrir los ojos seas tu,sintiendo tu calor, deslumbrándome mas que el mismo sol\nEspero que todos los días que me quedan sigan siendo a tu lado\nGracias por darme vida`;  } else {
...
function showSignature() {
  ...
  signature.textContent = firma ? decodeURIComponent(firma) : texto_firma;
  signature.classList.add('visible');
}

let texto_firma = "Te amo";
...
function showCountdown() {
  ...
  let startDate = startParam ? new Date(startParam + 'T00:00:00') : new Date('2025-11-27T00:00:00'); 
  let año_actual = new Date().getFullYear();
  let fecha_aniversario = new Date(''+año_actual+'-11-27T00:00:00')
  let fecha_actual=new Date();
  while(fecha_aniversario < fecha_actual ){
      año_actual = año_actual +1;
      fecha_aniversario = new Date(''+año_actual+'-11-27T00:00:00')
  }
  console.log(año_actual);
  let eventDate = eventParam ? new Date(eventParam + 'T00:00:00') : new Date(''+año_actual+'-11-27T00:00:00');
  ...
  function update() {
    ...
    if(eventDiff == 0){ texto_firma = "Feliz Aniversario Mi Amor";}
